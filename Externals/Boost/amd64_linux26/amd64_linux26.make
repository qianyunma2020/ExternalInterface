...
package=Boost
version=v0
PACKAGE_ROOT=$(BOOSTROOT)
srcdir=../src
bin=../$(Boost_tag)/
javabin=../classes/
mgrdir=cmt
BIN=/data/BOSSParallel/ExternalInterface/Externals/Boost/amd64_linux26/
project=ExternalInterface
cmt_installarea_paths= $(cmt_installarea_prefix)/$(CMTCONFIG)/bin
use_linkopts= $(cmt_installarea_linkopts)   $(Boost_linkopts)  $(Python_linkopts) 
...
use_requirements=requirements $(CMT_root)/mgr/requirements $(Python_root)/cmt/requirements 
use_includes= $(ppcmd)"$(Python_root)/src" 
use_fincludes= $(use_includes)
use_stamps=  $(Boost_stamps)  $(Python_stamps) 
use_cflags=  $(Boost_cflags)  $(Python_cflags) 
use_pp_cflags=  $(Boost_pp_cflags)  $(Python_pp_cflags) 
use_cppflags=  $(Boost_cppflags)  $(Python_cppflags) 
use_pp_cppflags=  $(Boost_pp_cppflags)  $(Python_pp_cppflags) 
use_fflags=  $(Boost_fflags)  $(Python_fflags) 
use_pp_fflags=  $(Boost_pp_fflags)  $(Python_pp_fflags) 
use_libraries= $(Python_libraries) 
includes= $(ppcmd)"/usr/include" $(use_includes)
fincludes= $(includes)
