...
package=jsoncpp
version=v0
PACKAGE_ROOT=$(JSONCPPROOT)
srcdir=../src
bin=../$(jsoncpp_tag)/
javabin=../classes/
mgrdir=cmt
BIN=/data/BOSSParallel/ExternalInterface/Externals/jsoncpp/amd64_linux26/
project=ExternalInterface
cmt_installarea_paths= $(cmt_installarea_prefix)/$(CMTCONFIG)/bin
use_linkopts= $(cmt_installarea_linkopts)   $(jsoncpp_linkopts) 
...
use_stamps=  $(jsoncpp_stamps) 
use_cflags=  $(jsoncpp_cflags) 
use_pp_cflags=  $(jsoncpp_pp_cflags) 
use_cppflags=  $(jsoncpp_cppflags) 
use_pp_cppflags=  $(jsoncpp_pp_cppflags) 
use_fflags=  $(jsoncpp_fflags) 
use_pp_fflags=  $(jsoncpp_pp_fflags) 
includes= $(ppcmd)"/data/BOSSParallel/jsoncpp/include" $(use_includes)
fincludes= $(includes)
